...
func _state_changed(to:STATE) -> void:
	match to:
		STATE.SELECTED:
			...
			tween.tween_property(bump, "position:x", -140, 0.3)
			tween.tween_interval(0.1)
			tween.tween_property(bump, "position:x", -120, 0.3)
			tween.tween_interval(0.1)
			
		STATE.UNSELECTED:
			#if tween and tween.is_running(): await tween.finished
			restart_tween().set_parallel() 
			tween.tween_property(bump, "position:x", -108, 0.4)
		...
		STATE.PRESSED:
			...
			tween.tween_property(bump, "position:x", -96, 0.1)
